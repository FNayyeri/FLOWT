        ...
        if tracking_base_dir.exists():
            ...
            with st.expander("🎨 Visualisation Options", expanded=False):
                ...
                with col2:
                    ...
                    current_font_size = config.get('video_generation', {}).get('font_size', 0.6)
                    ...
                    col_fontsize, col_fontsize_set= st.columns([1,5])
                    with col_fontsize:
                        st.markdown("Font Size")
                    with col_fontsize_set:
                        font_size = st.slider("Font Size", 0.3, 1.5, current_font_size, 0.1, label_visibility="collapsed")
                    
                    # Save changes to config if values changed
                    if (bbox_color != current_bbox_color or 
                        text_color != current_text_color or 
                        bbox_thickness != current_thickness or
                        ...
                        config['video_generation']['font_size'] = font_size
                        
                        with open(config_path, 'w') as f:
                            yaml.dump(config, f, default_flow_style=False)
                        
                        st.rerun()
            if Path(selected_tracking).exists():
                st.markdown(f"**Using Tracking Data:** {Path(selected_tracking).name}")
                ...
                # if True:
                    with st.spinner("Generating video with corrected detections..."):
                        try:
                            ...
                            tracking_path = selected_tracking
                ...
                st.warning("No tracking data found for this video. Please run tracking first.")
        else:
